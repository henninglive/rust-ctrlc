...
mod platform {
    ...
    impl Output {
        /// stores current piped stdout or creates a new cache
        ...
        /// Sets pipe as stdout/stderr or flush flushes cache to new stdout/stderr.
    ...
    /// We do this because GenerateConsoleCtrlEvent() send an event
    /// to all processes on the same console. We want it to be received
    /// only by our process.
    pub unsafe fn setup() -> io::Result<()> {
        ...
        // This breaks the rust's stdout for pre 1.18.0. Rust used to
    ...
    /// Print to both consoles, not thread safe
    pub unsafe fn print(fmt: ::std::fmt::Arguments) {
        use self::io::Write;
        {
